  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <Panel title="Cloudbeds connection" description="Connection health without exposing credentials or secrets."><div className="space-y-4 p-5">
        <Row label="SEM readiness" value={<StatusBadge status={connectionStatus}/>}/><Row label="Authorized" value={authorized ? 'Yes' : 'No'}/><Row label="Environment" value={status?.environment || runtime?.environment || 'sandbox'}/><Row label="Data status" value={status?.dataStatus || 'offline'}/><Row label="Property IDs" value={(status?.connectedPropertyIds || []).join(', ') || 'Not discovered'}/><Row label="Last sync" value={status?.lastSyncAt || 'No successful sync yet'}/><Row label="Two-way write scopes requested" value={writeScopes.length ? `${writeScopes.length} enabled in OAuth request` : 'None'}/>
    ...
    <Panel title="Cloudbeds permissions requested by SEM" description="Read and write scopes used by the operational PMS. Delete and Payment Write are intentionally excluded."><div className="grid gap-3 p-5 sm:grid-cols-2 xl:grid-cols-4">{requestedScopes.map((scope)=>{const missing=missingScopes.includes(scope);const write=String(scope).startsWith('write:');return <div key={scope} className={`rounded-xl border p-4 ${missing?'border-amber-200 bg-amber-50':write?'border-blue-200 bg-blue-50':'border-emerald-200 bg-emerald-50'}`}><div className={`font-mono text-xs font-bold ${missing?'text-amber-900':write?'text-blue-900':'text-emerald-900'}`}>{scope}</div><div className={`mt-2 text-[11px] ${missing?'text-amber-700':write?'text-blue-700':'text-emerald-700'}`}>{missing?'Permission needs attention':write?'Write-back permission':'Read permission'}</div></div>})}</div></Panel>

    <Panel title="Cloudbeds write audit" description="Who changed what, which endpoint was called, and whether Cloudbeds accepted it." action={<button onClick={loadAudit} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700">Refresh audit</button>}>
