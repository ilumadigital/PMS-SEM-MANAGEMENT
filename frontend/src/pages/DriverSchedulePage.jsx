...
const mapsSearch=(place)=>`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(place || '')}`;
const mapsRoute=(from,to)=>`https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(from || 'Airport')}&destination=${encodeURIComponent(to || '')}`;
...
const DriverSchedulePage=()=>{
  ...
  return <div className="space-y-6">
    <PageHeader title="My Shuttle Schedule" description="Only shuttles assigned to you are shown. Pickup is the airport and drop-off is the reservation property." actions={<div className="flex gap-2"><button onClick={()=>setWeekStart(addDays(weekStart,-7))} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-bold text-slate-700">‹ Week</button><button onClick={()=>setWeekStart(mondayOf(todayKey()))} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-bold text-slate-700">Today</button><button onClick={()=>setWeekStart(addDays(weekStart,7))} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-bold text-slate-700">Week ›</button></div>} />
          ...
          return <section key={day} className={`min-h-56 rounded-2xl border bg-white shadow-sm ${isToday?'border-blue-300 ring-2 ring-blue-100':'border-slate-200'}`}>
            ...
            <div className="space-y-3 p-3">
              {items.map(row=><article key={row.id} className="rounded-xl border border-slate-200 p-3">
                ...
                <div className="mt-3 space-y-2 text-xs"><div><span className="font-bold text-slate-500">Pickup:</span> Airport</div><div><span className="font-bold text-slate-500">Drop-off:</span> {row.destination || 'Property'}</div><div className="text-slate-500">{row.passengers || 1} guest(s) · {row.cabin_luggages || 0} cabin · {row.luggage || 0} luggage</div>{row.flight_info&&<div className="font-semibold text-slate-700">Flight {row.flight_info}</div>}</div>
