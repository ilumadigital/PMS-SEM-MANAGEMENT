...
const ReceptionDash = () => {
  ...
  const statusAction = async (reservation, nextStatus) => {
    ...
    try {
      ...
      setNotice(`${reservation.guestName}: ${nextStatus==='in_house'?'checked in':'checked out'} locally in SEM PMS.`);
    } catch { /* detailed context error */ }
    finally { setActiveId(null); }
  };

  return <div className="space-y-6">
    <PageHeader title="Front Desk" description="Today’s arrivals and departures. Check-in/out is managed locally in SEM PMS; Cloudbeds remains read-only." actions={<div className="flex flex-wrap gap-2"><button onClick={()=>navigate('/calendar')} className="rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50">Open Calendar</button><button onClick={()=>navigate('/calendar')} className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-blue-700">Calendar</button><button onClick={refresh} className="rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Refresh</button></div>} />

    <div className="rounded-2xl border border-blue-200 bg-gradient-to-r from-blue-50 to-white px-5 py-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-sm font-black text-blue-950">Cloudbeds read-only feed</div><div className="mt-1 text-xs leading-5 text-blue-800">Reservations, properties and rooms come from Cloudbeds. Operational status changes stay inside SEM PMS and are never written back.</div></div><button onClick={()=>navigate('/calendar?new=1')} className="shrink-0 rounded-xl bg-slate-950 px-4 py-3 text-sm font-black text-white">View calendar</button></div>
    </div>
    ...
    <Panel title="Guest movement" description="Check-in/out status is stored locally in SEM PMS only." action={<div className="flex flex-wrap gap-2">{[['today','Today'],['arrivals','Arrivals'],['departures','Departures'],['inhouse','In house']].map(([key,label])=><button key={key} onClick={()=>setView(key)} className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${view===key?'bg-slate-900 text-white':'border border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}>{label}</button>)}</div>}>
      ...
      {rows.length ? <TableShell><thead><tr><Th>Movement</Th><Th>Time</Th><Th>Guest</Th><Th>Room</Th><Th>Stay</Th><Th>Status</Th><Th>Info</Th><Th>PMS action</Th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map((reservation)=>{
