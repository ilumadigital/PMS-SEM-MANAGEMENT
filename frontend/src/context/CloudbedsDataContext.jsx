...
const applyLocalReservationOperations = (reservations, operations = []) => {
  ...
  return reservations.map((reservation) => {
    ...
    return {
      ...
      onlineCheckin: local.onlineCheckin == null ? Boolean(reservation.onlineCheckin) : Boolean(local.onlineCheckin),
      actualArrivalTime: local.actualArrivalTime || null,
      actualDepartureTime: local.actualDepartureTime || null,
      arrivalTime: local.actualArrivalTime || reservation.arrivalTime || null,
      departureTime: local.actualDepartureTime || reservation.departureTime || null,
      guestNotes: local.guestNotes ?? reservation.guestNotes ?? '',
...
const deriveLocalHousekeeping = (rooms, reservations, stored = []) => {
  ...
  return rooms.map((room) => {
    ...
    return {
      ...
      status: condition,
      refill: currentIsToday ? Boolean(current?.refill) : false,
      extraLinens: currentIsToday ? (current?.extraLinens || '') : '',
